...
Implements UnitTestOptionParser
	...
	#tag Method, Flags = &h0
		Sub Parse(argString As String, firstArgIsExecutable As Boolean = False)
		  dim args() as string
		  if firstArgIsExecutable then
		    args = ParseStringValue(argString)
		  else
		    ParseRestOfString argString, args
		  end if
		  
		  Parse args, FirstArgIsExecutable
		...
		Sub ShowHelp(sectionTitle As String = "Help")
		  ...
		  For i As Integer = 0 To Options.Ubound
		    ...
		    If opt.ShortKey <> "" Then
		      ...
		      if opt.CanReadValueFromPath then
		        keyString = keyString + "|@FILE"
		      end if
